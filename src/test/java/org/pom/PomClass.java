...
public class PomClass extends BaseClass {
	...
	public void atoz() {
		List<WebElement> itemname = driver.findElements(inventoryname);
		List<String> bfrfilter = new ArrayList<String>();
		for (WebElement p : itemname) {
			p.getText();
		}
		selectByVisibleText(filter, "Name (A to Z)");
		List<WebElement> afteritemname = driver.findElements(inventoryname);
	...
	public void hightolow() {
		List<WebElement> itemname = driver.findElements(inventoryItemPrice);
		...
		List<WebElement> afteritemname = driver.findElements(inventoryItemPrice);
